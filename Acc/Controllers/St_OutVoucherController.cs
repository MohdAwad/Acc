...
{
    [Authorize]
    public class St_OutVoucherController : BaseController
    {
  private readonly IUnitOfWork _unitOfWork;
        public St_OutVoucherController()
        {
        ...
        public ActionResult Index()
        {
         ...
         [HttpPost]
        public JsonResult GetAllSt_OutVoucher(St_HeaderVM Obj)
        {
            try
            {
                var userId = User.Identity.GetUserId();
                ...
                return Json(AllSt_OutVoucher, JsonRequestBehavior.AllowGet);
            }
            catch (Exception ex)
            {
        ...
        public JsonResult UpdateSt_OutVoucher(St_HeaderVM ObjToUpdate)
        {
            MsgUnit Msg = new MsgUnit();
            try
            {
        ...
        public JsonResult DeleteSt_OutVoucher(St_HeaderVM ObjToDelete)
        {
            MsgUnit Msg = new MsgUnit();
            try
            {
        ...
        public JsonResult GetAllUnExport(St_HeaderVM Obj)
        {
            try
            {
                var userId = User.Identity.GetUserId();
                var UserInfo = _unitOfWork.User.GetMyInfo(userId);
                var AllUnExport = _unitOfWork.NativeSql.GetAllSt_UnExport(UserInfo.fCompanyId, Obj.FromDate, Obj.ToDate, 505);
                if (AllUnExport == null)
                {
                    return Json(new List<St_HeaderVM>(), JsonRequestBehavior.AllowGet);
                }
        ...
        public JsonResult GetAllExport(St_HeaderVM Obj)
        {
            try
            {
                var userId = User.Identity.GetUserId();
                var UserInfo = _unitOfWork.User.GetMyInfo(userId);
                var AllUnExport = _unitOfWork.NativeSql.GetAllSt_Export(UserInfo.fCompanyId, Obj.FromDate, Obj.ToDate, 505);
                if (AllUnExport == null)
                {
                    return Json(new List<St_HeaderVM>(), JsonRequestBehavior.AllowGet);
                }
