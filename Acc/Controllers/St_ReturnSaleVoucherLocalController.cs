...
{
    [Authorize]
    public class St_ReturnSaleVoucherLocalController : BaseController
    {
        private readonly IUnitOfWork _unitOfWork;
        public St_ReturnSaleVoucherLocalController()
        {
        ...
        [HttpPost]
        public JsonResult GetAllSt_ReturnSaleVoucher(St_HeaderVM Obj)
        {
            try
            {
                var userId = User.Identity.GetUserId();
                ...
                return Json(AllSt_ReturnSaleVoucher, JsonRequestBehavior.AllowGet);
            }
            catch (Exception ex)
            {
        ...
        public ActionResult Add()
        {
            ...
            if (ObjGet != null)
            {
                ObjFix.AccountNo = ObjGet.SalesTaxReturnAccountNumber;
                ObjFix.AccountName = _unitOfWork.NativeSql.GetAccountName(UserInfo.fCompanyId, ObjGet.SalesTaxReturnAccountNumber);
            }
            var St_Warehouse = _unitOfWork.St_Warehouse.GetAllSt_Warehouse(UserInfo.fCompanyId);
        ...
        public JsonResult Save(St_HeaderVM ObjToSave)
        {
            ...
            try
            {
                ...
                {
                    ObjCredit.CreditForeign = ObjToSave.NetTotalLocal / Header.ConversionFactor;
                    ObjCredit.CreditDebitForeign = ObjToSave.NetTotalLocal / Header.ConversionFactor;
                }
                ...
                if (ObjToSave.TaxType == 1)
                {
                    ...
                    {
                        ObjTax.DebitForeign = ObjToSave.NetTotalLocal / Header.ConversionFactor;
                        ObjTax.CreditDebitForeign = ObjToSave.NetTotalLocal / Header.ConversionFactor;
                    }
        ...
        public JsonResult UpdateSt_ReturnSaleVoucher(St_HeaderVM ObjToUpdate)
        {
            MsgUnit Msg = new MsgUnit();
            try
            {
                ...
                {
                    ObjCredit.CreditForeign = ObjToUpdate.NetTotalLocal / Header.ConversionFactor;
                    ObjCredit.CreditDebitForeign = ObjToUpdate.NetTotalLocal / Header.ConversionFactor;
                }
                ObjCredit.DebitForeign = 0;
                ...
                if (ObjToUpdate.TaxType == 1)
                {
                    ...
                    {
                        ObjTax.DebitForeign = ObjToUpdate.NetTotalLocal / Header.ConversionFactor;
                        ObjTax.CreditDebitForeign = ObjToUpdate.NetTotalLocal / Header.ConversionFactor;
                    }
                    ObjTax.CreditForeign = 0;
                    _unitOfWork.Transaction.Add(ObjTax);
                }
        ...
        public JsonResult DeleteSt_ReturnSaleVoucher(St_HeaderVM ObjToDelete)
        {
            MsgUnit Msg = new MsgUnit();
            try
            {
        ...
        public JsonResult GetAllUnExport(St_HeaderVM Obj)
        {
            try
            {
                var userId = User.Identity.GetUserId();
                var UserInfo = _unitOfWork.User.GetMyInfo(userId);
                var AllUnExport = _unitOfWork.NativeSql.GetAllSt_UnExport(UserInfo.fCompanyId, Obj.FromDate, Obj.ToDate, 502);
                if (AllUnExport == null)
                {
                    return Json(new List<St_HeaderVM>(), JsonRequestBehavior.AllowGet);
                }
        ...
        public JsonResult GetAllExport(St_HeaderVM Obj)
        {
            try
            {
                var userId = User.Identity.GetUserId();
                var UserInfo = _unitOfWork.User.GetMyInfo(userId);
                var AllUnExport = _unitOfWork.NativeSql.GetAllSt_Export(UserInfo.fCompanyId, Obj.FromDate, Obj.ToDate, 502);
                if (AllUnExport == null)
                {
                    return Json(new List<St_HeaderVM>(), JsonRequestBehavior.AllowGet);
                }
