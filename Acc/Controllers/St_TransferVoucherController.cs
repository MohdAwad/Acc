...
{
    [Authorize]
    public class St_TransferVoucherController : BaseController
    {
        private readonly IUnitOfWork _unitOfWork;
        public St_TransferVoucherController()
        {
        ...
        [HttpPost]
        public JsonResult GetAllSt_TransferVoucher(St_HeaderVM Obj)
        {
            try
            {
                var userId = User.Identity.GetUserId();
                ...
                return Json(AllSt_TransferVoucher, JsonRequestBehavior.AllowGet);
            }
            catch (Exception ex)
            {
        ...
        public ActionResult Add()
        {
            ...
            St_HeaderVM Obj = new St_HeaderVM
            {
                ...
                St_WarehouseFrom = St_Warehouse,
                St_WarehouseTo = St_Warehouse,
                StockCode = "*",
                CurrencyID = 1,
                ConversionFactor = 1,
                TransactionKindNo = 507,
                TaxType = 3,
                VoucherCase = 1,
        ...
        public JsonResult Save(St_HeaderVM ObjToSave)
        {
            ...
            try
            {
                ...
                SaveHeader.FromStockCode = ObjToSave.FromStockCode;
                SaveHeader.ToStockCode = ObjToSave.ToStockCode;
                ...
                int iRow = 0;
                foreach (var SaveItems in ObjToSave.St_TransactionFrom)
                {
                    if (!String.IsNullOrWhiteSpace(SaveItems.ItemCode))
                    {
                        try
                        {
                            ...
                            var GetAverageCostByStock = ObjFunction.GetAvgCostByStock(UserInfo.fCompanyId, SaveItems.ItemCode, ObjToSave.FromStockCode, SaveHeader.VoucherDate);
                            ...
                            SaveItems.StockCode = ObjToSave.FromStockCode;
                ...
                foreach (var SaveItems in ObjToSave.St_TransactionTo)
                {
                    if (!String.IsNullOrWhiteSpace(SaveItems.ItemCode))
                    {
                        try
                        {
                            ...
                            SaveItems.CompanyTransactionKindNo = SaveHeader.CompanyTransactionKindNo;
                            SaveItems.TransactionKindNo = SaveHeader.TransactionKindNo;
                            SaveItems.StockCode = ObjToSave.ToStockCode;
                            SaveItems.VoucherDate = SaveHeader.VoucherDate;
                            SaveItems.VoucherNumber = SaveHeader.VoucherNumber;
                            SaveItems.VHI = SaveHeader.VHI;
                            SaveItems.IsDeleted = 0;
        ...
        public ActionResult Update(string id, int id2, int id3, int id4, string id5)
        {
            ...
            var St_HeaderVM = new St_HeaderVM { };
            St_HeaderVM.St_WarehouseFrom = St_Warehouse;
            St_HeaderVM.St_WarehouseTo = St_Warehouse;
            St_HeaderVM.FromStockCode = St_HeaderObj.FromStockCode;
            St_HeaderVM.ToStockCode = St_HeaderObj.ToStockCode;
        ...
        public JsonResult UpdateSt_TransferVoucher(St_HeaderVM ObjToUpdate)
        {
            MsgUnit Msg = new MsgUnit();
            try
            {
                ...
                SaveHeader.FromStockCode = ObjToUpdate.FromStockCode;
                SaveHeader.ToStockCode = ObjToUpdate.ToStockCode;
                ...
                _unitOfWork.NativeSql.DeleteSt_Transaction(SaveHeader.CompanyID, SaveHeader.VoucherNumber, SaveHeader.CompanyTransactionKindNo, SaveHeader.TransactionKindNo, SaveHeader.CompanyYear, "0");
                _unitOfWork.Complete();
                foreach (var SaveItems in ObjToUpdate.St_TransactionFrom)
                {
                    if (!String.IsNullOrWhiteSpace(SaveItems.ItemCode))
                    {
                        try
                        {
                            iRow = iRow + 1;
                            FunctionUnit ObjFunction = new FunctionUnit();
                            var GetAverageCostByStock = ObjFunction.GetAvgCostByStock(UserInfo.fCompanyId, SaveItems.ItemCode, ObjToUpdate.FromStockCode, SaveHeader.VoucherDate);
                            SaveItems.CompanyID = UserInfo.fCompanyId;
                            SaveItems.CompanyYear = SaveHeader.CompanyYear;
                            SaveItems.CompanyTransactionKindNo = SaveHeader.CompanyTransactionKindNo;
                            SaveItems.TransactionKindNo = SaveHeader.TransactionKindNo;
                            SaveItems.StockCode = ObjToUpdate.FromStockCode;
                            SaveItems.VoucherDate = SaveHeader.VoucherDate;
                            SaveItems.VoucherNumber = SaveHeader.VoucherNumber;
                            SaveItems.VHI = SaveHeader.VHI;
                            SaveItems.IsDeleted = 0;
                ...
                foreach (var SaveItems in ObjToUpdate.St_TransactionTo)
                {
                    if (!String.IsNullOrWhiteSpace(SaveItems.ItemCode))
                    {
                        try
                        {
                            ...
                            SaveItems.CompanyTransactionKindNo = SaveHeader.CompanyTransactionKindNo;
                            SaveItems.TransactionKindNo = SaveHeader.TransactionKindNo;
                            SaveItems.StockCode = ObjToUpdate.ToStockCode;
                            SaveItems.VoucherDate = SaveHeader.VoucherDate;
                            SaveItems.VoucherNumber = SaveHeader.VoucherNumber;
                            SaveItems.VHI = SaveHeader.VHI;
                            SaveItems.IsDeleted = 0;
        ...
        public JsonResult DeleteSt_TransferVoucher(St_HeaderVM ObjToDelete)
        {
            MsgUnit Msg = new MsgUnit();
            try
            {
                ...
                _unitOfWork.NativeSql.DeleteSt_Transaction(DeleteHeader.CompanyID, DeleteHeader.VoucherNumber, DeleteHeader.CompanyTransactionKindNo, DeleteHeader.TransactionKindNo, DeleteHeader.CompanyYear, "0");
                if (!ModelState.IsValid)
                {
                    string Err = " ";
                    var errors = ModelState.Values.SelectMany(v => v.Errors);
        ...
        public JsonResult GetAllUnExport(St_HeaderVM Obj)
        {
            try
            {
                var userId = User.Identity.GetUserId();
                var UserInfo = _unitOfWork.User.GetMyInfo(userId);
                var AllUnExport = _unitOfWork.NativeSql.GetAllSt_UnExport(UserInfo.fCompanyId, Obj.FromDate, Obj.ToDate, 507);
                if (AllUnExport == null)
                {
                    return Json(new List<St_HeaderVM>(), JsonRequestBehavior.AllowGet);
                }
        ...
        public JsonResult GetAllExport(St_HeaderVM Obj)
        {
            try
            {
                var userId = User.Identity.GetUserId();
                var UserInfo = _unitOfWork.User.GetMyInfo(userId);
                var AllUnExport = _unitOfWork.NativeSql.GetAllSt_Export(UserInfo.fCompanyId, Obj.FromDate, Obj.ToDate, 507);
                if (AllUnExport == null)
                {
                    return Json(new List<St_HeaderVM>(), JsonRequestBehavior.AllowGet);
                }
        ...
        [HttpGet]
        public JsonResult GetAverageCost(string id, string id2,string id3)
        {
            if (id != "")
            {
                DateTime VoucherDate = DateTime.Parse(id3.Substring(0, 2) + "/" + id3.Substring(2, 2) + "/" + id3.Substring(4, 4));
                var userId = User.Identity.GetUserId();
                var UserInfo = _unitOfWork.User.GetMyInfo(userId);

                FunctionUnit ObjFunction = new FunctionUnit();
                var GetAverageCostByStock = ObjFunction.GetAvgCostByStock(UserInfo.fCompanyId, id, id2, VoucherDate);
                if (GetAverageCostByStock == null)
                {
                    return Json("", JsonRequestBehavior.AllowGet);
                }
                else
                {
                    return Json(GetAverageCostByStock, JsonRequestBehavior.AllowGet);
                }
            }
            else
            {
