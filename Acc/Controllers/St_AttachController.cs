...
namespace Acc.Controllers
{
    ...
    public class St_AttachController : BaseController
    {
        private readonly IUnitOfWork _unitOfWork;

        public St_AttachController()
        {
        ...
        public FileResult DownloadDocument(int id, string id2, string id3, string id4, int id5)//nagham task
        {
            ...
            St_TransActionFile DocumentID = _unitOfWork.St_TransActionFile.GetTransActionFilebyId(sfileid);




            string path = Server.MapPath("/St_TransActionStockFiles/" + Year + "/" + VoucherNumber + "/" + TransactionKindNo + "/" + CompanyTransactionKindNo + "/");
            byte[] fileBytes = System.IO.File.ReadAllBytes(path + DocumentID.FileName);
            string sfileName = DocumentID.FileName;
            return File(fileBytes, System.Net.Mime.MediaTypeNames.Application.Octet, sfileName);
        }
        ...
        public ActionResult SaveSt_TransActionFile(int id, string id2, string id3, string id4)
        {
            int Year = id;
            string VoucherNumber = id2;
            string TransactionKindNo = id4;
            ...
            try
            {
                ...
                string path = Path.Combine(new DirectoryInfo(string.Format(@"{0}St_TransActionStockFiles\{1}\{2}\{3}\{4}", base.Server.MapPath(@"\"), Year, VoucherNumber, TransactionKindNo, CompanyTransactionKindNo)).ToString());
                if (!Directory.Exists(path))
                {
                    Directory.CreateDirectory(path);
                }
                foreach (string str4 in base.Request.Files)
                {
                    ...
                    if ((base2 != null) && (base2.ContentLength > 0))
                    {
                        ...
                        string sFileName = base2.FileName.Substring(0, base2.FileName.IndexOf('.'));
                        sFileName = Guid.NewGuid().ToString() + extension;
                        base2.SaveAs(Path.Combine(base.Server.MapPath(String.Format("~/St_TransActionStockFiles/{0}/{1}/{2}/{3}", Year, VoucherNumber, TransactionKindNo, CompanyTransactionKindNo)), sFileName));




                        St_TransActionFile objSave = new St_TransActionFile
                        {
                            FileName = sFileName,
                            Year = Year,
                        ...
                        this._unitOfWork.St_TransActionFile.Add(objSave);
                        this._unitOfWork.Complete();
        ...
        public ActionResult UploadSt_TransActionFile(int id, string id2, string id3, string id4)
        {
        ...
        public JsonResult GetAllTRansFiles(int id, string id2, string id3, string id4)
        {
            try
            {
                var userId = User.Identity.GetUserId();
                var UserInfo = _unitOfWork.User.GetMyInfo(userId);
                var Type = _unitOfWork.St_TransActionFile.GetType(UserInfo.fCompanyId);
                ...
                var AllTRansFile = _unitOfWork.St_TransActionFile.GetAllTransactionFile(Year, VoucherNumber, TransactionKindNo, CompanyTransactionKindNo);
                if (AllTRansFile == null)
                {
                    return Json(new List<St_TransActionFile>(), JsonRequestBehavior.AllowGet);
                }
        ...
        public ActionResult DeleteAttach(int id)
        {
            try
            {
                ...
                if (id != 0)
                {
                    ...
                    var Obj = _unitOfWork.St_TransActionFile.GetTransActionFilebyId(id);
                ...
                return PartialView("DeleteFile", new St_TransActionFile());
            }
            catch (Exception ex)
            {
                ViewBag.Error = ex.Message.ToString();
        ...
        public JsonResult DeleteAttachById(int id)
        {
            ...
            try
            {
                ...
                string path = Path.Combine(new DirectoryInfo(string.Format(@"{0}St_TransActionStockFiles\{1}\{2}\{3}\{4}\{5}", base.Server.MapPath(@"\"), Obj.Year, Obj.VoucherNumber, Obj.TransactionKindNo, Obj.CompanyTransactionKindNo, Obj.FileName)).ToString());
                _unitOfWork.St_TransActionFile.Delete(id);
                _unitOfWork.Complete();
                if (System.IO.File.Exists(path))
                {
                    // If file found, delete it    
